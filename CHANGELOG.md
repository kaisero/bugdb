...
### Added
- **The data refresh can be dispatched as a full re-crawl.** Running the
  workflow manually now offers a `full_fetch` option that re-crawls every
  product from scratch. A scheduled refresh is incremental and can only add
  version keys it has not seen — it cannot update or remove one that already
  exists — so corrected data for an existing version is parsed and then
