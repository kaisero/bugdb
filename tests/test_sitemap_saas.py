...
def test_discover_saas_urls_finds_ai_runtime_security():
    ...
    assert any(
        "ai-runtime-security/release-notes/addressed-issues" in u for u in addressed
    )
...
def test_discover_saas_urls_uses_correct_cloud_ngfw_aws_prefix():
    ...
    sitemap = SitemapIndex.from_xml(_SITEMAP)
    known, addressed = discover_saas_urls(sitemap, "cloud-ngfw-aws")
    assert any(
        "cloud-ngfw-aws/release-notes/cloud-ngfw-for-aws-known-issues" in u
        for u in known
    )
...
def test_discover_saas_urls_finds_cloud_ngfw_azure():
    ...
    assert any(
        "cloud-ngfw-azure/release-notes/cloud-ngfw-for-azure-known-issues" in u
        for u in known
    )
    assert any(
...
def test_discover_saas_urls_honours_manifest_skip():
    ...
    known, addressed = discover_saas_urls(
        sitemap, "ai-runtime-security", manifest=manifest
    )
    assert known == []
    assert any(
        "ai-runtime-security/release-notes/addressed-issues" in u for u in addressed
