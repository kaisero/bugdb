...
class PrismaAccessAgentCrawler(BaseCrawler):
    ...
    async def discover_versions(self) -> list[str]:
        ...
        async def check_version(version: str) -> str | None:
            ...
            url = "/prisma-access-agent/release-notes"
            try:
        ...
        logger.debug(
            "Discovered %d Prisma Access Agent versions: %s", len(sorted_versions), sorted_versions
        )
    ...
    async def crawl(
        ...
        for major_version, version_infos in vi_by_major.items():
            ...
            logger.info("Crawling Prisma Access Agent %s...", version_str)
