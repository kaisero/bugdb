...
async def dispatch_async(product_id: str, *args, **kwargs) -> FetchResult:
    ...
    If ``PRODUCT_WRAPPERS[product_id]`` has been monkey-patched (e.g. by
    a unit test using ``patch.dict``), the patched wrapper is called via
    ``asyncio.to_thread`` so the test's mock fires. The async dispatch
    is only used when the wrapper is the original — that's the hot path
...
# ==============================================================================
# PRODUCT_WRAPPERS: single source of truth mapping product_id -> sync wrapper
# ==============================================================================
#
# Every crawler that PRODUCT_CRAWLERS lists MUST also have an entry here.
# The invariant is enforced by tests/unit/test_registry.py — if you add a
# product to PRODUCT_CRAWLERS without adding it here, the test fails.
#
# cli.supported_products is derived from this dict, so adding a product to
# both PRODUCT_CRAWLERS and PRODUCT_WRAPPERS is enough to wire it up through
# the CLI. (The deeper refactor that collapses the ~20 hand-written
# _crawl_*_async / crawl_* functions above is roadmap D5, deferred to
# v1.1.0.)
...
# Frozen snapshot of PRODUCT_WRAPPERS as registered at import time. Used by
# ``dispatch_async`` to tell whether a per-product entry in PRODUCT_WRAPPERS
# is still the original sync wrapper (use the async dispatch — keeps the
# shared httpx Transport bound to one loop) or has been monkey-patched by a
# test (call the patched value via ``asyncio.to_thread`` so the mock fires).
_ORIGINAL_PRODUCT_WRAPPERS: dict[str, Callable[..., FetchResult]] = dict(PRODUCT_WRAPPERS)
